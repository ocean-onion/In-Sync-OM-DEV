...
def test_start_game():
    ...
    if function_name == "typingprint" or function_name == "colourprint" or function_name == "typingprint_nl" or function_name == "colourprint_nl":
        ...
        seconds = int(input("Enter seconds to count down (1-25 recommended): "))
        result = function(seconds)
    elif function_name == "check_user_deck":
        deck_str = input("Enter a comma-separated list of cards (e.g., 1,5,10): ")
        player = {
            "name": "Test Player",
        ...
        for i in range(num_players):
            deck_str = input(f"Enter a comma-separated list of cards for Player {i+1}: ")
            player = {
                "name": f"Player {i+1}",
                "deck": [int(c.strip()) for c in deck_str.split(",") if c.strip()]
            }
        ...
        for i in range(num_players):
            ...
            deck_str = input(f"Enter a comma-separated list of cards for {name}: ")
            player = {
                "name": name,
                "deck": [int(c.strip()) for c in deck_str.split(",") if c.strip()]
            }
    ...
    elif function_name == "check_win_condition":
        num_players = int(input("Enter number of players (2-4): "))
        players = []
        for i in range(num_players):
            deck_str = input(f"Enter a comma-separated list of cards for Player {i+1}: ")
        ...
        function()
    elif function_name == "loading_screen" or function_name == "shuffle_screen":
        function()
        ...
        run_it = input("Do you want to run this entire function? (y/n): ").lower()
        if run_it == 'y':
            result = function()
        else:
        ...
        try:
            ...
            print(f"Error: {e}")
            print("This function requires parameters that aren't handled by the test utility.")
            return

    print(f"{color.BLUE}Function completed.")

    test_function_q = input(f"{color.END}Do you want to test another function? (y/n): ").lower()
...
def test_colors():
    ...
    if option == "A":
        ...
        print("Enter comma-separated color numbers to combine (e.g., 1,8 for RED and BOLD):")
