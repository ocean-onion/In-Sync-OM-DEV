import random
from utils.utilities import colourprint, typingprint, typinginput, cards, clear, wait, instructions, color, countdown
from utils.dev import execute_dev_command
...
def play_card(player, card):
    ...
    typingprint(
        f"{GREEN}{player['name']} has successfully played the card {BOLD}{card}{END}{GREEN}.{END}"
    )
    return card
    ...
    while True:
        ...
        if previous_card is not None:
            ...
            if len(played_cards) > 1:
                cards_display = ", ".join(
                    [f"{YELLOW}{card}{END}" for card in played_cards])
                typingprint(f"{CYAN}Cards played so far:{END} {cards_display}")
        else:
            typingprint("{YELLOW}{BOLD}No cards have been played yet.{END}")

        typingprint(
            "{BLUE}{UNDERLINE}Anyone can play a card at any time.{END}")

        chosen_user = typinginput("{PURPLE}Enter your name: {END}")

        if chosen_user.startswith("!?"):
            command = chosen_user[2:]
            execute_dev_command(command, players, played_cards)
            continue

        chosen_card_input = typinginput("{CYAN}Choose a card to play: {END}")
            ...
            if player["name"].lower() == chosen_user.lower():
                ...
                if check_user_deck(player, chosen_card):
                    if check_lowest_possible(players, chosen_card):
                        ...
                        typingprint("{CYAN}Remaining cards per player:{END}")
                        for p in players:
                            typingprint(
                                f"{BLUE}{p['name']}:{END} {YELLOW}{len(p['deck'])}{END} cards"
                            )
                    else:
                        remaining_cards = get_all_remaining_cards(players)
                        lowest_card = min(remaining_cards)
                        correct_player = find_correct_player(
                            players, lowest_card)
                        ...
                        cards_display = ", ".join(
                            [f"{CYAN}{card}{END}" for card in played_cards])
                        typingprint(
                            f"{PURPLE}Cards played in order before the error:{END} {cards_display}"
                        )
                        return
                break
...
def styled_start_game():
    clear()
    typingprint(f"{YELLOW}{BOLD}Welcome to The Mind{END}")
    typingprint(
