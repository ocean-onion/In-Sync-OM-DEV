...
def game_loop(players):
    while True:
        ...
        for player in players:
            ...
            if not chosen_card_input.isdigit() or int(chosen_card_input) <= 0:
            ...
            if check_lowest_possible(players, chosen_card):
                ...
                typingprint(
                    f"{player['name']} played {chosen_card} successfully!")
...
def start_game():
    ...
    while True:
        ...
        if not num_players_input.isdigit() or not (2 <= int(num_players_input) <= 4):
            typingprint(
